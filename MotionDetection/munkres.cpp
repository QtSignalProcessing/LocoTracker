...
unsigned int minsize(cv::Mat_<double> &m)
{
    return (m.rows < m.cols) ? m.rows : m.cols;
}

int maxValue(cv::Mat_<double> &m)
{
...
void extendMat(cv::Mat_<double> &m, unsigned int rows, unsigned int cols, int value = 0)
{
    cv::Size2i interSize;
...
void replace_infinites(cv::Mat_<double> &matrix)
{
    ...
    const double infinity = std::numeric_limits<int>::infinity();
...
void minimize_along_direction(cv::Mat_<double> &matrix, bool over_columns)
{
 ...
 *
 */
void Munkres::solve(const cv::Mat_<double> &m)
{
